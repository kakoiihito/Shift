...
func _update_suspension_forces() -> void:
	var suspension = car.suspension
	_suspension_forces.clear()
	for i in range(suspension.wheel_spring_force.size()):
		var spring_force_mag = suspension.wheel_spring_force[i].length()
		_suspension_forces.append(snappedf(spring_force_mag, 0.01))
func _update_label() -> void:
	...
	label.text = "RPM: %.0f\nClutch Torque: %.2f\nEngine Torque: %.2f\nGear: %d\nClutch Engagement: %.2f\nThrottle: %.2f\nLongitude Force: %s\nMax Traction Force: %s\nTraction Force: %.2f\nSuspension Force: %s" % [rpm, clutch_torque, engine_torque, current_gear, clutch_engagement, throttle_input, str(_rounded_forces), str(_max_traction_forces), _traction_force, str(_suspension_forces)]
