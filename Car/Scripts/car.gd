...
func _physics_process(delta: float) -> void:
	Transmission.transmission_process(delta, transmission, VehicleValues) # independent
	
	for wheel in wheels:
		Suspension.suspension_proccess(wheel, suspension, car, VehicleValues) # independent
	
	Steering.steering_proccess(delta, steering, wheeldata, car, VehicleValues) # independent
	Brake.brake_process(delta, brake, VehicleValues) # independent
	

	for wheel in wheels:
		if VehicleValues.TireModel == VehicleValues.TireModelType.MF52_Lite:
			MF52_LiteProcess._get_wheel_forces(wheel, wheeldata, suspension, car, VehicleValues) # requires wheel angular velocity, motor, suspension
		elif VehicleValues.TireModel == VehicleValues.TireModelType.MF52_Full:
			MF52_FullProcess._get_wheel_forces(wheel, wheeldata, suspension, car, VehicleValues) # requires wheel angular velocity, motor, suspension
		elif VehicleValues.TireModel == VehicleValues.TireModelType.Brush_Model:
			BrushModel_Process._get_wheel_forces(wheel, wheeldata, suspension, car, VehicleValues) # requires wheel angular velocity, motor, suspension
		
	Motor.motor_process(delta, engine, transmission, wheeldata, VehicleValues) # requires wheel angular velocity, wheel forces, transmission
	
	for wheel in wheels:
		if VehicleValues.TireModel == VehicleValues.TireModelType.MF52_Lite:
			MF52_LiteProcess._get_wheel_angular_velocity(wheel, delta, wheeldata, engine, brake, suspension, car, VehicleValues) # requires wheel forces, suspension, motor, brakes
		elif VehicleValues.TireModel == VehicleValues.TireModelType.MF52_Full:
